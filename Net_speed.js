...
// 替换为自定义节点（示例：测速网）
  let up = {'url': 'https://www.speedtest.cn/api/upload', timeout:3000}
// 下载节点需携带字节数参数（模拟原逻辑的bytes参数）
  let down = {'url': `https://www.speedtest.cn/api/download?size=${bytes}`, timeout:3000}
// 延迟测试节点
  let cp = {'url': `https://www.speedtest.cn/api/latency`, timeout:3000}
  // 兼容性修正
  if ($.isLoon()) {
  ...
  // 构造面板
  let Panel = {};
  if ($.isStash()) Panel.title = arg?.title ?? "NetSpeed"
  else Panel.title = arg?.title ?? "NetSpeed"
  if ($.isLoon()) {
      Panel.message = `------------------------------\n`
      + `${round(Math.abs(speed * 8))} Mbps\n`
      + `${round(Math.abs(speed, 2), 1)} MB/s\n`
      + `耗时: ${round(Math.abs(duration, 2),2)}s\n`
